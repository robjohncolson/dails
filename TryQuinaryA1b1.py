...
def get_free_space(device):
    ...
    for line in lines:
        ...
        if line.startswith("Disk "):
            parts = line.split()
            if len(parts) >= 3:  # Ensure there are enough parts to access
                try:
...
def suggest_partition_table(total_size, device):
...
#        "sudo parted {device} mklabel msdos && "
    ...
    ).format(device=device, efi_size=efi_size, root_size=root_size, swap_size=swap_size)
...
def main():
    ...
    partition_cmd = suggest_partition_table(total_size, device)  # Pass device to the function
    ...
    print("Displaying the following command to create the partition table:")
    print(partition_cmd)
    #pause here
